					 ...
					 uint64_t *cycles)
{
	...
	prescaler = ll_gptim_get_prescaler(config->tim);
...
static void set_channel_ccxs(const struct device *dev, uint32_t channel, uint32_t ccxs_val)
{
	...
	ll_gptim_set_channel_direction(tim, channel + 1U, ccxs_val);
}
...
static void set_channel_polarity(const struct device *dev, uint32_t channel, bool inverted)
{
	...
	ll_gptim_set_channel_polarity(tim, channel + 1U, inverted ? 1U : 0U);
}
				...
				pwm_flags_t flags)
{
	...
	switch (channel) {
	case 0:
		ll_gptim_set_trigger_source(config->tim, LL_GPTIM_TRIGGER_SRC_TI1FP1);
		ll_gptim_set_slave_mode(config->tim, LL_GPTIM_SLAVE_MODE_RESET);
		break;
	case 1:
		ll_gptim_set_trigger_source(config->tim, LL_GPTIM_TRIGGER_SRC_TI2FP2);
		ll_gptim_set_slave_mode(config->tim, LL_GPTIM_SLAVE_MODE_RESET);
		break;
	default:
		break;
...
static int pwm_sf32lb_enable_capture(const struct device *dev, uint32_t channel)
{
	...
	ll_gptim_clear_update_flag(tim);
	ll_gptim_clear_channel_flag(tim, 1);
	ll_gptim_clear_channel_flag(tim, 2);
	ll_gptim_clear_channel_flag(tim, 3);
	ll_gptim_clear_channel_flag(tim, 4);

	ll_gptim_enable_channel_interrupt(tim, channel + 1U);
	ll_gptim_enable_update_interrupt(tim);

	ll_gptim_enable_channel(tim, channel + 1U);

	if (comp_idx < MAX_CH_NUM && comp_idx != channel) {
		ll_gptim_enable_channel(tim, comp_idx + 1U);
	}
	return 0;
}
...
static int pwm_sf32lb_disable_capture(const struct device *dev, uint32_t channel)
{
	...
	ll_gptim_disable_channel_interrupt(tim, channel + 1U);
	ll_gptim_disable_update_interrupt(tim);
	ll_gptim_disable_channel(tim, channel + 1U);
	ll_gptim_clear_channel_flag(tim, channel + 1U);

	comp_idx = complementary_channel[channel] - 1U;
	if (comp_idx < MAX_CH_NUM && comp_idx != channel) {
		ll_gptim_disable_channel(tim, comp_idx + 1U);
		ll_gptim_clear_channel_flag(tim, comp_idx + 1U);
	}

	if (channel < 2U) {
		ll_gptim_set_slave_mode(tim, LL_GPTIM_SLAVE_MODE_DISABLED);
		ll_gptim_set_trigger_source(tim, 0U);
	}
...
static void pwm_sf32lb_isr(const struct device *dev)
{
	...
	/* Update (overflow) */
	if (ll_gptim_get_update_flag(tim)) {
		ll_gptim_clear_update_flag(tim);
		cpt->overflows++;
	}

	/* Capture event for channel */
	if (ll_gptim_get_channel_flag(tim, channel + 1U)) {
		uint32_t period = ll_gptim_get_capture(tim, channel + 1U);
		uint32_t comp_idx = complementary_channel[channel] - 1U;
		uint32_t pulse = ll_gptim_get_capture(tim, comp_idx + 1U);
		ll_gptim_clear_channel_flag(tim, channel + 1U);
