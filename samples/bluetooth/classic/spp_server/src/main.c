...
#define BOARD_SPP_UUID_VAL 0x3001
static struct bt_rfcomm_server rfcomm_server2;
static struct bt_rfcomm_dlc rfcomm_dlc2;

/*
 * Outbound DLC used when this board initiates an spp connect.
 */
static struct bt_rfcomm_dlc rfcomm_client_dlc;

static void addr_to_str(const bt_addr_t *addr, char *buf, size_t len);
static void spp_start_sdp_discover(struct bt_conn *conn);

/*
 * Current peer address (recorded in the ACL connected callback); used as a
 * simple sanity check by the send_file command.
 */
static bt_addr_t g_peer_addr;
static bool g_peer_valid;

/*
 * Peer addresses of the two board-to-board links: g_board_addr matches
 * rfcomm_dlc2 (incoming), g_client_addr matches rfcomm_client_dlc (outgoing).
 */
static bt_addr_t g_board_addr;
static bool g_board_valid;
static bt_addr_t g_client_addr;
static bool g_client_valid;

/*
 * Select the connected DLC matching the peer address (phone / board incoming /
 * board outgoing).
 */
static struct bt_rfcomm_dlc *pick_dlc_for_addr(const bt_addr_t *addr)
{
	if (g_peer_valid && rfcomm_dlc.session != NULL && bt_addr_cmp(addr, &g_peer_addr) == 0) {
		return &rfcomm_dlc;
	}
	if (g_board_valid && rfcomm_dlc2.session != NULL && bt_addr_cmp(addr, &g_board_addr) == 0) {
		return &rfcomm_dlc2;
	}
	if (g_client_valid && rfcomm_client_dlc.session != NULL &&
	    bt_addr_cmp(addr, &g_client_addr) == 0) {
		return &rfcomm_client_dlc;
	}

	if (g_peer_valid && rfcomm_dlc.session != NULL) {
		return &rfcomm_dlc;
	}
	if (g_board_valid && rfcomm_dlc2.session != NULL) {
		return &rfcomm_dlc2;
	}
	if (g_client_valid && rfcomm_client_dlc.session != NULL) {
		return &rfcomm_client_dlc;
...
/*
 * transfer is complete after this long without new data
 */
#define FILE_RX_IDLE_TIMEOUT_MS 500U
/*
 * print progress after every this many received bytes
 */
...
/*
 * Use the uptime in ms as the file name so names stay unique per power-on.
 */
static void file_rx_start(void)
{
	...
	if (rc < 0) {
		/*
		 * When storage is unavailable, only count without saving; reception keeps
		 * working.
		 */
...
static void file_rx_feed(const uint8_t *data, uint32_t len)
{
	...
	/*
	 * Each received packet postpones the idle-timeout timer; the summary is only
	 * printed after data actually stops for FILE_RX_IDLE_TIMEOUT_MS.
	 */
...
/*
 * Board-to-board SDP record for the custom UUID, bound to rfcomm_server2.channel.
 */
static struct bt_sdp_attribute board_sdp_attrs[] = {
	BT_SDP_NEW_SERVICE,
	BT_SDP_LIST(
		BT_SDP_ATTR_SVCLASS_ID_LIST,
		BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 3),
		...
		{
			BT_SDP_TYPE_SIZE(BT_SDP_UUID16),
			BT_SDP_ARRAY_16(BOARD_SPP_UUID_VAL)
		},
		)
	),
	BT_SDP_LIST(
		...
		},
		{
			...
			{
				BT_SDP_TYPE_SIZE(BT_SDP_UINT8),
				&rfcomm_server2.channel
			},
			)
		},
		)
	),
	BT_SDP_SERVICE_NAME("Board SPP"),
};

static struct bt_sdp_record board_sdp_rec = BT_SDP_RECORD(board_sdp_attrs);

/*
 * Dynamic SDP client discovery used by spp search/connect (arbitrary UUID).
 */
...
{
	const char *role = (dlc == &rfcomm_dlc) ? "phone" :
		(dlc == &rfcomm_dlc2) ? "board(accepted)" : "board(connected out)";

	LOG_INF("RFCOMM connected (%s)", role);
}
...
static void rfcomm_disconnected(struct bt_rfcomm_dlc *dlc)
{
	...
	if (dlc == &rfcomm_dlc) {
		g_peer_valid = false;
	} else if (dlc == &rfcomm_dlc2) {
		g_board_valid = false;
	} else if (dlc == &rfcomm_client_dlc) {
		g_client_valid = false;
	}

	/*
	 * Settle any in-progress file reception immediately when the link drops.
	 */
	if (g_rx_stat.active) {
...
/*
 * Packets larger than this are treated as file/bulk data (no echo, no reply).
 */
			 ...
			 struct bt_rfcomm_dlc **dlc)
{
	...
	if (bt_conn_get_info(conn, &info) == 0 && info.type == BT_CONN_TYPE_BR && info.br.dst) {
...
/*
 * Accept for the board-to-board custom UUID channel, into rfcomm_dlc2/g_board_addr.
 */
static int rfcomm_accept2(struct bt_conn *conn, struct bt_rfcomm_server *server,
			  struct bt_rfcomm_dlc **dlc)
{
	struct bt_conn_info info;
	...
	if (rfcomm_dlc2.session != NULL) {
		return -ENOMEM;
	}

	if (bt_conn_get_info(conn, &info) == 0 && info.type == BT_CONN_TYPE_BR && info.br.dst) {
		bt_addr_copy(&g_board_addr, info.br.dst);
		g_board_valid = true;
	}

	rfcomm_dlc2.ops = &rfcomm_ops;
	rfcomm_dlc2.mtu = RFCOMM_MTU;
	rfcomm_dlc2.required_sec_level = BT_SECURITY_L0;
	*dlc = &rfcomm_dlc2;
...
static void acl_connected(struct bt_conn *conn, uint8_t err)
{
	...
	/*
	 * If this ACL link was triggered by spp search/connect, continue with the SDP
	 * discovery for the requested UUID.
	 */
...
static void acl_disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	/*
	 * Only clear the peer state when the address matches, to avoid wiping other
	 * links.
	 */
...
/*
 * All callbacks empty => Just Works pairing, no PIN required.
 */
...
static void bt_ready(int err)
{
	...
	rfcomm_server2.accept = rfcomm_accept2;

	err = bt_rfcomm_server_register(&rfcomm_server2);
	if (err != 0) {
		LOG_ERR("Board RFCOMM server register failed (err %d)", err);
		return;
	}

	err = bt_sdp_register_service(&board_sdp_rec);
	if (err != 0) {
		LOG_ERR("Board SDP service register failed (err %d)", err);
		bt_rfcomm_server_unregister(&rfcomm_server2);
		return;
	}

	LOG_INF("Board SPP (uuid 0x%04x) registered on RFCOMM channel %u",
		BOARD_SPP_UUID_VAL, rfcomm_server2.channel);
...

	{
		...
		if (bt_br_oob_get_local(&oob) == 0) {
			...
			LOG_INF("My BD address: %s (board-to-board: on the peer board run "
				"`spp connect %s 2 3001` to reach my 0x%04x channel)",
				astr, astr, BOARD_SPP_UUID_VAL);
		}
	}
}
...
static int cmd_spp_send_data(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (argc < 3) {
		shell_error(sh, "usage: spp send_data <bd_addr> <service_channel>");
		return -EINVAL;
	}
	...
	dlc = pick_dlc_for_addr(&addr);
	if (dlc == NULL) {
		shell_error(sh, "no active RFCOMM connection to %s", argv[1]);
		return -ENOTCONN;
	}
	...
	rc = bt_rfcomm_dlc_send(dlc, tx_buf);
	if (rc < 0) {
		net_buf_unref(tx_buf);
...
static int cmd_spp_send_file(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (argc < 4) {
		shell_error(sh, "usage: spp send_file <bd_addr> <service_channel> <file_path>");
		return -EINVAL;
	}
	...
	/*
	 * Cap the chunk size by the negotiated MTU to avoid rejected sends.
	 */
	{
		size_t chunk_cap = sizeof(buf);

		if (dlc->mtu > 0 && dlc->mtu < chunk_cap) {
			chunk_cap = dlc->mtu;
		}
		shell_print(sh, "negotiated RFCOMM MTU = %u, chunk size = %u",
			    dlc->mtu, (unsigned int)chunk_cap);
	}
	...
	while (sent < (uint32_t)stat.size) {
		...
		/*
		 * bt_rfcomm_create_pdu() blocks when tx_pool is exhausted, giving
		 * backpressure.
		 */
		tx_buf = bt_rfcomm_create_pdu(&tx_pool);
		if (!tx_buf) {
				   ...
				   const struct bt_sdp_discover_params *params)
{
	...
	if (g_pending.auto_connect) {
		if (rfcomm_client_dlc.session != NULL) {
			LOG_WRN("[SPP CONNECT] client dlc already in use, abort");
		} else {
			rfcomm_client_dlc.ops = &rfcomm_ops;
			rfcomm_client_dlc.mtu = RFCOMM_MTU;
			rfcomm_client_dlc.required_sec_level = BT_SECURITY_L0;

			err = bt_rfcomm_dlc_connect(conn, &rfcomm_client_dlc, (uint8_t)channel);
			if (err != 0) {
				LOG_ERR("[SPP CONNECT] RFCOMM connect failed (err %d)", err);
			} else {
				bt_addr_copy(&g_client_addr, &g_pending.addr);
				g_client_valid = true;
...
static int cmd_spp_addr(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_print(sh, "board SPP: uuid 0x%04x, RFCOMM channel %u",
		    BOARD_SPP_UUID_VAL, rfcomm_server2.channel);
	return 0;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(spp_cmds,
	SHELL_CMD_ARG(addr, NULL,
		      "addr -- print local BD address and board SPP channel",
		      cmd_spp_addr, 1, 0),
